...
func TestCollectPublic_TypeMismatchWarns(t *testing.T) {
	home := t.TempDir()
	if err := os.MkdirAll(filepath.Join(home, "skills"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if len(col.Entries) != 0 || len(col.Warnings) == 0 {
		t.Fatalf("expected type-mismatch warning + no entries, got entries=%v warnings=%v", col.Entries, col.Warnings)
	}
}
